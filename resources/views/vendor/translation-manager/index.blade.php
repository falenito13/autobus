        ...
        <div class="container-fluid">
            <div class="row bg-title">
                <div class="col-lg-3 col-md-4 col-sm-4 col-xs-12">
                    <h4 class="page-title">{{ Lang::get('global.translations')}}</h4> </div>
                <div class="col-lg-9 col-sm-8 col-md-8 col-xs-12">
                    <ol class="breadcrumb">
                        <li><a href="#">Admin</a></li>
                        <li class="active">{{ Lang::get('global.translations')}}</li>
                    </ol>
                </div>
            </div>
                <div class="row">
                    <div class="col-md-12">
                        <div class="white-box">
                            <div class="row">
                                <div class=".col-md-6">
                                <p>{!! Lang::get('global.trans_warning') !!}  </p>
                                <div class="alert alert-success success-import" style="display:none;">
                                    <p>{!! Lang::get('global.done_importing') !!}</p>
                                </div>
                                <div class="alert alert-success success-find" style="display:none;">
                                    <p>{!! Lang::get('global.done_searching') !!}</p>
                                </div>
                                <div class="alert alert-success success-publish" style="display:none;">
                                    <p>{!! Lang::get('global.done_publishing') !!} '<?php echo $group ?>'!</p>
                                </div>
                                <div class="alert alert-success success-publish-all" style="display:none;">
                                    <p>{!! Lang::get('global.done_publishing_all') !!}</p>
                                </div>
                                        ...
                                        <div class="row">
                                            <div class="col-sm-3">
                                                <select name="replace" class="form-control">
                                                    <option value="0">{{ Lang::get('global.append_new_tr') }}</option>
                                                    <option value="1">{{ Lang::get('global.replace_ex') }}</option>
                                                </select>
                                            </div>
                                            <div class="col-sm-2">
                                                <button type="submit" class="btn btn-success btn-block"  data-disable-with="{{ Lang::get('global.loading') }}..">{{ Lang::get('global.import_groups') }}</button>
                                            </div>
                                        </div>
                                    </div>
                                </form>
                                <form class="form-find" method="POST" action="<?php echo action('\Barryvdh\TranslationManager\Controller@postFind') ?>" data-remote="true" role="form" data-confirm="Are you sure you want to scan you app folder? All found translation keys will be added to the database.">
                                    <div class="form-group">
                                        <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
                                        <button type="submit" class="btn btn-info" data-disable-with="Searching.." >{{ Lang::get('global.find_tr_infiles') }}</button>
                                    </div>
                                </form>
                                <?php endif; ?>
                                <?php if(isset($group)) : ?>
                                <form class="form-inline form-publish" method="POST" action="<?php echo action('\Barryvdh\TranslationManager\Controller@postPublish', $group) ?>" data-remote="true" role="form" data-confirm="Are you sure you want to publish the translations into JS?">
                                    <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
                                    <button type="submit" class="btn btn-info" data-disable-with="{!! Lang::get('global.publishing') !!}.." >{{ Lang::get('global.publish_tr') }}</button>
                                    <a href="<?= action('\Barryvdh\TranslationManager\Controller@getIndex') ?>" class="btn btn-default">{{ Lang::get('global.back') }}</a>
                                </form>
                                <?php endif; ?>
                                </p>
                                <form role="form" method="POST" action="<?php echo action('\Barryvdh\TranslationManager\Controller@postAddGroup') ?>">
                                    <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
                                    <div class="form-group">
                                        <p>{!! Lang::get('global.chose_tr_group') !!}</p>
                                    ...
                                    <div class="form-group">
                                        <label>{!! Lang::get('global.enter_new_tr_group') !!}</label>
                                        <input type="text" class="form-control" name="new-group" />
                                    </div>
                                    <div class="form-group">
                                        <input type="submit" class="btn btn-default" name="add-group" value="{!! Lang::get('global.add_edit_tr_keys') !!}" />
                                    </div>
                                </form>
                                <?php if($group): ?>
                                <form action="<?php echo action('\Barryvdh\TranslationManager\Controller@postAdd', array($group)) ?>" method="POST"  role="form">
                                    <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
                                    <div class="form-group">
                                        <label>{!! Lang::get('global.add_tr_key') !!}</label>
                                        <textarea class="form-control" rows="3" name="keys" placeholder="Add 1 key per line, without the group prefix"></textarea>
                                    </div>
                                    <div class="form-group">
                                        <input type="submit" value="{!! Lang::get('global.add_key') !!}" class="btn btn-primary">
                                    </div>
                                </form>
                                <div class="row">
                                    <div class="col-sm-2">
                                        <span class="btn btn-default enable-auto-translate-group">{!! Lang::get('global.use_auto_tr') !!}</span>
                                    </div>
                                </div>
                                <form class="form-add-locale autotranslate-block-group hidden" method="POST" role="form" action="<?php echo action('\Barryvdh\TranslationManager\Controller@postTranslateMissing') ?>">
                                    ...
                                    <div class="row">
                                        <div class="col-sm-6">
                                            <div class="form-group">
                                                <label for="base-locale">{!! Lang::get('global.base_locale') !!}</label>
                                            ...
                                            <div class="form-group">
                                                <label for="new-locale">{!! Lang::get('global.target_key') !!}</label>
                                                <input type="text" name="new-locale" class="form-control" id="new-locale" placeholder="{!! Lang::get('global.target_key') !!}" />
                                            </div>
                                            <?php if(!config('laravel_google_translate.google_translate_api_key')): ?>
                                            <p>
                                                <code>{!! Lang::get('global.google_warning') !!}</code>
                                            </p>
                                            <?php endif; ?>
                                            <div class="form-group">
                                                <input type="hidden" name="with-translations" value="1">
                                                <input type="hidden" name="file" value="<?= $group ?>">
                                                <button type="submit" class="btn btn-default btn-block"  data-disable-with="{!! Lang::get('global.adding') !!}..">{!! Lang::get('global.auto_tr_ms') !!}</button>
                                            </div>
                                        </div>
                                    </div>
                                </form>
                                <hr>
                                <h4>{!! Lang::get('global.total') !!}: <?= $numTranslations ?>, {!! Lang::get('global.changed') !!}: <?= $numChanged ?></h4>
                                <table class="table">
                                    <thead>
                                    <tr>
                                        <th width="15%">Key</th>
                                ...
                                <?php else: ?>
                                <fieldset>
                                    <legend>{!! Lang::get('global.sup_locales') !!}</legend>
                                    <p>
                                        {!! Lang::get('global.locales') !!}:
                                    </p>
                                    ...
                                    <form class="form-add-locale" method="POST" role="form" action="<?php echo action('\Barryvdh\TranslationManager\Controller@postAddLocale') ?>">
                                        ...
                                        <div class="form-group">
                                            <p>
                                                {!! Lang::get('global.enter_new_locale_key') !!}:
                                            </p>
                                            <div class="row">
                                                <div class="col-sm-3">
                                                ...
                                                <div class="col-sm-2">
                                                    <button type="submit" class="btn btn-default btn-block"  data-disable-with="Adding..">{!! Lang::get('global.add_new_locale') !!}</button>
                                                </div>
                                            </div>
                                        </div>
                                    </form>
                                ...
                                <fieldset>
                                    <legend>{!! Lang::get('global.export_all_tr') !!}</legend>
                                    <form class="form-inline form-publish-all" method="POST" action="<?php echo action('\Barryvdh\TranslationManager\Controller@postPublish', '*') ?>" data-remote="true" role="form" data-confirm="Are you sure you want to publish all translations group? This will overwrite existing language files.">
                                        <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
                                        <button type="submit" class="btn btn-primary" data-disable-with="{!! Lang::get('global.publishing') !!}.." >{!! Lang::get('global.publish_all') !!}</button>
                                    </form>
                                </fieldset>
            ...
            <script>
                jQuery(document).ready(function($){
                    ...
                    $('.form-publish').on('ajax:success', function () {
                        $.get("{{ route('langstojs') }}", function(){
                            console.log('transfered into JS file!!!');
                        });
